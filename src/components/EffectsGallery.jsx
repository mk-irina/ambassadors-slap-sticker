...
export const EffectsGallery = ({setEffect}) => {
    ...
    return (
      ...
      {effects.map((effect, idx) => {
          return (
          <button onClick={() => onSetEffect(effect.name)} key={idx} className="size-28 shadow-sm bg-white rounded-md flex-shrink-0">
            <img src={effect.url} alt="slap" className="h-full w-full object-cover rounded-md"/>
          </button>
          )
        })}
      </div>
